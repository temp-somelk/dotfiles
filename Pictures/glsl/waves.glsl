...
{
    return 3.29284291400159 - 0.85373472095314 * r;
}
...
float pattern(in vec2 p) {
    ...
    vec2 r = vec2( fbm( p + 8.0*q + vec2(1.7,9.2),oc,l,g ), fbm( p + 4.0*q + vec2(3.2
										 ,2.8) ,oc,l,g));
...
void main() {
    ...
    vec2 qq;
    vec2 r;
    float color = pattern2(p,qq,r,time*0.0005);
    
    vec4 c = vec4(color*0.5,color*0.9,color*0.7,color);
    c *= 3.5;
